...
class RBNet(object):
    """
    rb_net implements the robust balance representation learning for counterfactual inference
    ...
    def _build_discriminator_graph_mine(self, x, hrep, dim_input, dim_in, dim_mi, flags):
        ...
        with tf.variable_scope('gmi') as scope:
            ...
            weights_mi_pred = self._create_variable(tf.random_normal([dim_mi, 1],
                                                                     stddev=flags.weight_init / np.sqrt(dim_mi)),
                                                    'gmi_p')
    ...
    def _build_adversarial_graph(self, rep, t, dim_in, dim_d, do_out, flags):
        ...
        d0, weights_dis, weights_discore = self._build_discriminator_adversarial(z_rep0_conc, dim_in + dim_in, dim_d,
                                                                                 do_out, flags)
        d1, weights_dis, weights_discore = self._build_discriminator_adversarial(z_rep1_conc, dim_in + dim_in, dim_d,
                                                                                 do_out, flags, reuse=True)
