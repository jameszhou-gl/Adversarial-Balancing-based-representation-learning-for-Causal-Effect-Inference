...
def train(ABNet, sess, train_step, train_discriminator_step, train_encoder_step, train_hrep_step, D, I_valid, D_test,
          logfile, i_exp):
    ...
    dict_factual = {ABNet.x: D['x'][I_train, :], ABNet.t: D['t'][I_train, :], ABNet.y_: D['yf'][I_train, :], \
                    ABNet.do_in: 1.0, ABNet.do_out: 1.0, ABNet.r_lambda: FLAGS.p_lambda, ABNet.r_beta: FLAGS.p_beta,
                    ABNet.p_t: p_treated, ABNet.z_norm: z_norm}
    ...
    obj_loss, f_error, gmi_err, discriminator_loss, rep_loss, gradient_pen = \
        sess.run(
            [ABNet.tot_loss, ABNet.pred_loss, ABNet.gmi_neg_loss, ABNet.discriminator_loss, ABNet.rep_loss, ABNet.dp], \
    ...
    if FLAGS.val_part > 0:
        valid_obj, valid_f_error, valid_gmi, valid_dc, valid_rep_r, valid_dp = \
            sess.run([ABNet.tot_loss, ABNet.pred_loss, ABNet.gmi_neg_loss, ABNet.discriminator_loss, ABNet.rep_loss,
                      ABNet.dp],
    ...
    for i in range(FLAGS.iterations):
            ...
            if not objnan:
                ...
                for sub_dc in range(0, 3):
                    sess.run(train_discriminator_step,
                             feed_dict={ABNet.x: x_batch, ABNet.t: t_batch, ABNet.r_beta: FLAGS.p_beta,
                                        ABNet.do_in: FLAGS.dropout_in, ABNet.do_out: FLAGS.dropout_out,
                                        ABNet.z_norm: z_norm_batch})
                # train encoder
                ...
                sess.run(train_step, feed_dict={ABNet.x: x_batch, ABNet.t: t_batch, \
                                                ABNet.y_: y_batch, ABNet.do_in: FLAGS.dropout_in,
                                                ABNet.do_out: FLAGS.dropout_out, \
        ...
        if i % FLAGS.output_delay == 0 or i == FLAGS.iterations - 1:
            ...
            if FLAGS.val_part > 0:
                valid_obj, valid_f_error, valid_gmi, valid_dc, valid_rep_r, valid_dp = \
                    sess.run(
                        [ABNet.tot_loss, ABNet.pred_loss, ABNet.gmi_neg_loss, ABNet.discriminator_loss, ABNet.rep_loss,
                         ABNet.dp],
                        feed_dict=dict_valid)
        ...
        if (FLAGS.pred_output_delay > 0 and i % FLAGS.pred_output_delay == 0) or i == FLAGS.iterations - 1:
            ...
            if D_test is not None:
                y_pred_f_test = sess.run(ABNet.output, feed_dict={ABNet.x: D_test['x'],
                                                                  ABNet.t: D_test['t'], ABNet.do_in: 1.0,
                                                                  ABNet.do_out: 1.0})
                y_pred_cf_test = sess.run(ABNet.output, feed_dict={ABNet.x: D_test['x'],
                                                                   ABNet.t: 1 - D_test['t'], ABNet.do_in: 1.0,
                                                                   ABNet.do_out: 1.0})
...
def run(outdir):
    ...
    for i_exp in range(1, n_experiments + 1):
        ...
        losses, preds_train, preds_test, reps, reps_test = \
            train(ABNet, sess, train_step, train_discriminator_step, train_encoder_step, train_hrep_step, D_exp,
                  I_valid,
