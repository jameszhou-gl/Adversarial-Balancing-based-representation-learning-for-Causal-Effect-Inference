...
def run(cfg_file, num_runs):
    # todo can be delete when in production env
    shutil.rmtree('results/ihdp/')
    os.mkdir('results/ihdp')

    configs = load_config(cfg_file)
    outdir = configs['outdir'][0]
    used_cfg_file = os.path.join(outdir, 'used_configs.txt')
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print('Usage: python abcei_param_search.py <config file> <num runs>')
    else:
